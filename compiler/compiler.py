...
class Class(Element):
    ...
    def _postprocess(self, service): 
        for attr in self.attrs:
            attr.parent = self 
            if attr.get:
                self.autogen(service, attr, "_autogen_%s_get_%s" % (self.name, attr.name), attr.type, ("_proxy", self))
            if attr.set:
                self.autogen(service, attr, "_autogen_%s_set_%s" % (self.name, attr.name), t_void, ("_proxy", self), ("value", attr.type))
        for method in self.methods:
            method.parent = self 
            self.autogen(service, method, "_autogen_%s_%s" % (self.name, method.name), method.type, ("_proxy", self), *[(arg.name, arg.type) for arg in method.args])
        self.parent = self
